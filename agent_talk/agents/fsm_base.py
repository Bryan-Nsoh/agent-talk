...
from agent_talk.core.coords import encode_cells_delta16, coords_to_bytes
...
class FiniteStateAgent:
    ...
    def grid_with_blocks(self, extra_blocks: Iterable[Coordinate]) -> Grid:
    ...
    def plan_path(self, extra_blocks: Iterable[Coordinate] = ()) -> List[Coordinate] | None:
        grid = self.grid_with_blocks(extra_blocks)
    ...
    def min_cut(self, extra_blocks: Set[Coordinate]) -> List[Coordinate]:
        union_grid = self.grid_with_blocks(extra_blocks)
        return min_vertex_cut_potential(union_grid, self.config.start, self.config.goal)

    def encode_cut(self, cells: Sequence[Coordinate]) -> tuple[str, int]:
